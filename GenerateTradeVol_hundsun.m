...
path_current = [path_goal 'currentHolding\' AccountInfo{i}.NAME '\' sdate '\'];
...
file_modle = [path_goal 'currentHolding\modle.xlsx'];
file_today = [path_current 'trade_p0'];
...
for i = 1:numOfTicker
    unionHolding(i,2) = tHolding(tHolding(:,1) == unionHolding(i,1), 2);
    unionHolding(i,3) = cHolding(cHolding(:,1) == unionHolding(i,1), 2);
end
diffHolding = [unionHolding(:,1) unionHolding(:,2) - unionHolding(:,3)];
fid = fopen(file_trade, 'w');
fprintf(fid_d, [repmat('%15d\t',1,size(diffHolding,2)), '\n'], diffHolding');
fclose(fid);
...
if copyfile(file_model, file_today,'f') == 1
