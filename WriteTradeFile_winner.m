...
for ipart = 1:N_PART
    ...
    for i = 1:numOfTrade
        if child_vol(i,ipart) < 0
            snum = snum + 1;
            fprintf(sfid, '%06d,%s,%d,%s,%s,%d\n', diffHolding(i,1), stock_name, abs(child_vol(i,ipart)), '卖二价','买二价' ,snum);
        elseif child_vol(i,ipart) > 0
            bnum =bnum + 1;
            fprintf(bfid, '%06d,%s,%d,%s,%s,%d\n', diffHolding(i,1), stock_name, abs(child_vol(i,ipart)), '卖二价','买二价', bnum);
        end
    end
    fclose(sfid);
    fclose(bfid);
    ...
    dst_sfile_today = [dir_client_account 'HistoricalTrade\' sfile_name '_' num2str(idate) '_' num2str(itime) '.csv'];
    dst_bfile_today = [dir_client_account 'HistoricalTrade\' bfile_name '_' num2str(idate) '_' num2str(itime) '.csv'];
